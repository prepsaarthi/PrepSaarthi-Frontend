...
export default function OTPVerification() {
  ...
  const { user: mentor, isAuthenticated: mAuth,loading:mloading } = useSelector(
    (state) => state.mentor
  );
  const { user: stuUser, isAuthenticated: sAuth, loading:sloading } = useSelector(
    (state) => state.student
  );
  const {
    loading: reLoading,
  ...
  >
    ...
    >
      <Typography variant="p" sx={{ fontSize: "1.4vmax" }} component="div">
      ...
      <LoadingButton
        ...
        sx={resend ? { display: "flex", m: "0 auto" } : { display: "none" }}
      >
        Resend OTP
      </LoadingButton>
    </Box>
